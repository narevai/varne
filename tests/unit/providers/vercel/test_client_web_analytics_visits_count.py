...
policy_vercel_web_analytics_visits_count = SanitizePolicy(
    root=FieldPolicy(
        keep=(
            "version",
            "query",
...
def vcr_config():
    return {
        ...
        "before_record_response": partial(
            sanitize_response, policy=policy_vercel_web_analytics_visits_count
        ),
    }

@pytest.mark.default_cassette("web_analytics_visits_count.yaml")
@pytest.mark.vcr
def test_fetch_web_analytics_visits_count(
    ...
    response = client.fetch_web_analytics_visits_count(
